...
public class SearchActivity extends PermissionActivity implements PoiSearch.OnPoiSearchListener {
    ...
    private void initView() {
        ...
        mRvAddressSearchTextAdapter = new RvAddressSearchTextAdapter(SearchActivity.this,mDatas);
        recycleView.setAdapter(mRvAddressSearchTextAdapter);
        mRvAddressSearchTextAdapter.setItemListener(new RecycleViewItemListener() {
            @Override
...
//                    mDatas.add(mAddressTextFirst);// 第一个元素
                    AddressSearchTextEntity addressEntity;
                    for (int i=0;i<poiItems.size();i++) {
                        PoiItem poiItem = poiItems.get(i);
                        if(i==0){
                            addressEntity = new AddressSearchTextEntity(true,poiItem.getLatLonPoint(),poiItem.getSnippet(),poiItem.getTitle());
                        }else{
                ...
                ToastUtil.show(SearchActivity.this, "对不起，没有搜索到相关数据！");
            }
        }
    }
